...
from packages.companyData import extract_companyLocationSalary, extract_companyLocationNoSalary
from packages.get_url import get_all_urls
...
if __name__ == "__main__":
    ...
    # <<<<<<< change PATH >>>>>>>>
    base_url, target_num = load_configs(path="/Users/irena/Desktop/scraper/glassdoor-scraper/scraper_glassdoor/data/config.json")
    ...
    # <<<<<<< change PATH and country>>>>>>>>
    country = "United Kingdom"
    output_fileName = "/Users/irena/Desktop/scraper/glassdoor-scraper/scraper_glassdoor/output/output_" + now.strftime("%d-%m-%Y") + "_" + str(target_num) + "_" + country + ".csv"
    
    csv_header = [("companyName", "company_starRating", "job_title", "employee_residence", "listing_jobDesc", "salary", "requested_url", "company_location", "company_size")]
    ...
    while total_listingCount <= target_num:
        ...
        for listing_url in listings_set:
            ...
            # only get jobs with salary
            if str(returned_tuple[5]) != "NA":
                try:
                    returned_tuples = extract_companyLocationSalary(returned_tuple[6])
                except: 
                    returned_tuples = ("NA", "NA")
                
                final_data = returned_tuple + returned_tuples
                list_returnedTuple.append(final_data)
